	...
	private static class URIHandlerWithCheck extends URIHandlerImpl {
		...
		public InputStream createInputStream(URI uri, Map<?, ?> options) throws IOException {
			try {
				...
				String md5 = null;
				try {
					md5 = getDigest(uriStringMD5, DigestUtil.MESSAGE_DIGESTERS[0].getDigestLength() << 1);
				}
				...
				String sha1 = null;
				try {
					sha1 = getDigest(uriStringSHA1, DigestUtil.MESSAGE_DIGESTERS[1].getDigestLength() << 1);
				}
				catch(FileNotFoundException e) {
